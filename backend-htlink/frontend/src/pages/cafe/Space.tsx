...
const CafeSpace: React.FC = () => {
  ...
  const loadInitialData = async () => {
    try {
      ...
      const [languages, settings, spaceData] = await Promise.all([
        ...
        spaceRequest<Space[]>('/cafe/spaces'),
      ]);
  ...
  const loadSpaces = async () => {
    try {
      const data = await spaceRequest<Space[]>('/cafe/spaces');
      setSpaces(data);
    } catch (error: any) {
      toast.error(error.message || 'Failed to load spaces');
    }
  ...
  const handleSave = async () => {
    ...
    try {
      setSaving(true);
      if (editingSpace.id) {
        await spaceRequest(`/cafe/spaces/${editingSpace.id}`, {
          method: 'PUT',
          headers: {
        ...
        await spaceRequest('/cafe/spaces', {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
          },
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      await spaceRequest(`/cafe/spaces/${id}`, {
        method: 'DELETE',
      });
  ...
  const persistSpaceOrder = useCallback(async (orderedSpaces: Space[]) => {
    await spaceRequest('/cafe/spaces/reorder', {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
      },
      body: JSON.stringify(orderedSpaces.map((space) => space.id)),
    });
  }, []);
                ...
                >
                  <div className="flex items-center gap-4">
                    ...
                    <div className="flex gap-3">
                      <button onClick={() => handleEdit(space)} className="flex items-center gap-2 rounded-md border border-slate-600 px-4 py-2 text-slate-600 transition-colors hover:bg-slate-50">Edit</button>
                      <button onClick={() => void handleDelete(space.id)} className="flex items-center gap-2 rounded-md bg-red-600 px-4 py-2 text-white transition-colors hover:bg-red-700">
