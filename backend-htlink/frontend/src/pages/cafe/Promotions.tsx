...
const CafePromotions: React.FC = () => {
  ...
  const loadInitialData = async () => {
    try {
      setLoading(true);
      const [promotionData, languages, settings, branchData, scenes] = await Promise.all([
      ...
      setPanoramaTargetId(settings.vr360_sections?.promotions?.target_id || '');
      setPanoramaUrl(settings.vr360_sections?.promotions?.panorama_url || '');
      setVr360Link(settings.vr360_sections?.promotions?.vr360_link || '');
      setVrTitleTranslations(getScopedTitleTranslations({
        promotions_title_translations: settings.vr360_sections?.promotions?.title_translations || {},
      }, 'promotions', locales));
  ...
  const persistVR360Change = async (field: 'target' | 'panorama' | 'vr' | 'title', value: string, localeForTitle = currentLocale) => {
    try {
      ...
      if (field === 'target') {
        updates.promotions_panorama_target_id = normalizedTargetId || null;
        updates.promotions_panorama_url = normalizedTargetId ? selectedScene?.panorama_url || null : null;
        setPanoramaTargetId(normalizedTargetId);
        setPanoramaUrl(normalizedTargetId ? selectedScene?.panorama_url || '' : '');
      } else if (field === 'panorama') {
        updates.promotions_panorama_url = value;
        ...
        Object.assign(updates, applyScopedTitleTranslations(updates, 'promotions', nextTranslations));
      ...
      <div className={SECTION_CLASS}>
        ...
        <div className="space-y-6">
          <div>
            ...
            <select className={FIELD_CLASS} value={getVr360TargetSelectValue(panoramaTargetId)} onChange={(e) => void handleVR360Change('target', e.target.value)} disabled={savingVR}>
          ...
          <div>
            ...
            <input type="url" className={FIELD_CLASS} value={panoramaUrl} onChange={(e) => void handleVR360Change('panorama', e.target.value)} placeholder="https://example.com/panorama.jpg" disabled={savingVR} />
          </div>
          <div>
            <label className={LABEL_CLASS}>VR URL</label>
            <input type="url" className={FIELD_CLASS} value={vr360Link} onChange={(e) => void handleVR360Change('vr', e.target.value)} placeholder="https://youtube.com/watch?v=... or https://example.com/vr" disabled={savingVR} />
          </div>
          <div>
            ...
            <input type="text" className={FIELD_CLASS} value={vrTitleTranslations[currentLocale] || ''} onChange={(e) => void handleVR360Change('title', e.target.value)} placeholder="Enter title" disabled={savingVR} />
          </div>
