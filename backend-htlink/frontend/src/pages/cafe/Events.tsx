...
const CafeEvents: React.FC = () => {
  ...
  const loadInitialData = async () => {
    try {
      setLoading(true);
      const [eventData, languageCodes, settings, branchData, scenes] = await Promise.all([
      ...
      setPanoramaTargetId(settings.vr360_sections?.events?.target_id || '');
      setPanoramaUrl(settings.vr360_sections?.events?.panorama_url || '');
      setVr360Link(settings.vr360_sections?.events?.vr360_link || '');
      setVrTitleTranslations(getScopedTitleTranslations({
        events_title_translations: settings.vr360_sections?.events?.title_translations || {},
      }, 'events', locales));
  ...
  const persistVR360Change = async (field: 'target' | 'panorama' | 'vr' | 'title', value: string, localeForTitle = currentLocale) => {
    try {
      ...
      if (field === 'target') {
        updates.events_panorama_target_id = normalizedTargetId || null;
        updates.events_panorama_url = normalizedTargetId ? selectedScene?.panorama_url || null : null;
        setPanoramaTargetId(normalizedTargetId);
        setPanoramaUrl(normalizedTargetId ? selectedScene?.panorama_url || '' : '');
      } else if (field === 'panorama') {
        updates.events_panorama_url = value;
        ...
        Object.assign(updates, applyScopedTitleTranslations(updates, 'events', nextTranslations));
